...
int main() {
    ...
    for (int i = 1; i <= 3; ++i) {
        std::string key = std::string("../conf/geo_index_test/index_") + std::to_string(i) + ".json";
    ...
    query["gendor"] = "female";
    query["student"] = true;
    query["location"] = GeoRange(116.400693, 39.907688, 20000);

    for (auto &p : query) {
        std::cout << "[PARAMS]" << p.print() << std::endl;
    }
